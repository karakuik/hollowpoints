...
exports.handler=async event=>{
  ...
  const {data,error}=await db.from('first_blood_matches').select('match_id,game_created_at,participants:match_data->participants').contains('participant_ids',[identity]).order('game_created_at',{ascending:false}).limit(1000)
